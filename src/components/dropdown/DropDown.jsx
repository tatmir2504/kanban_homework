	 ...
	 const remItem = (e) => {
		...
		<div  className="arrow"  >
			...
			<div className='dropdown' >
		       ...
		       <p key={title.id}  id = {title.id}  
			   title ={title.title} data-tag = {title.status}   
			   data-des = {title.description} data-desc = 'title' className='title'  
			   onClick={(e) => !isEdit?onClick(e):remItem(e)}>{title.title}</p>
				)}         
	         ...
	         {type === LIST_TYPES.IN_PROGRESS && read.map(title => 
			  ...
			  data-des = {title.description} className='title'   
			  onClick={(e) => !isEdit?onClick(e):remItem(e)}>{title.title}</p>
				)}  
			  ...
			  <p key={title.id} id = {title.id}  
			  title ={title.title} data-tag = {title.status} 
			  data-desc = 'description'  data-des = {title.description} className='title' 
			  onClick={(e) => !isEdit?onClick(e):remItem(e)} >{title.title}</p>
				)}  
			</div> 
