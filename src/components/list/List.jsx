...
const List = props => {
	...
	return (
		...
		<div className= "list"  >
			...
			{type === LIST_TYPES.READY &&  <button  className={ back.length === 0  ? " add" : "addButton"}   
			onClick={handleClick} disabled= {back.length === 0 } style={{ display: !isFormVisible ? "block" : "none" }} >+ Add card</button>}

			{type === LIST_TYPES.IN_PROGRESS &&  <button  className={ read.length === 0  ? " add" : "addButton"}   
			onClick={handleClick} disabled= {read.length === 0 } style={{ display: !isFormVisible ? "block" : "none" }} >+ Add card</button>}
			
			{type === LIST_TYPES.FINISHED && <button  className={ inProgres.length === 0  ? " add" : "addButton"}   
			onClick={handleClick} disabled= {inProgres.length === 0 } style={{ display: !isFormVisible ? "block" : "none" }} >+ Add card</button>}
